...
var INSTEONMessageFlags = module.exports = function INSTEONMessageFlags(flags){
	if(!(this instanceof INSTEONMessageFlags)){ 
		return new INSTEONMessageFlags(flags);
	}
	if(typeof flags == "number"){
		...
		flags = {
			...
			hopsLeft: util.makeBitmask(arr.slice(2, 4)) || 0,
...
INSTEONMessageFlags.prototype.valueOf = function valueOf(){
	var flags = this;
...
INSTEONMessageFlags.prototype.inspect = function inspect(){
	return return require("util").inspect({
		...
		hopsLeft: this.hopsLeft
	});
};
INSTEONMessageFlags.prototype.toString = function toString(radix){
	return this.valueOf().toString(radix);
};
INSTEONMessageFlags.prototype.toPrecision = function toPrecision(precision){
	return this.valueOf().toPrecision(precision);
};
INSTEONMessageFlags.prototype.toLocaleString = function toLocaleString(locales, options){
	return this.valueOf().toLocaleString(locales, options);
};
INSTEONMessageFlags.prototype.toFixed = function toFixed(digits){
	return this.valueOf().toFixed(digits);
};
INSTEONMessageFlags.prototype.toExponential = function toExponential(fractionDigits){
