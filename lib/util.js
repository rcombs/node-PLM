	...
	parseBitmask: function(nMask){
		...
		if(Buffer.isBuffer(nMask) || Array.isArray(nMask)){
			...
			for(var i = 0; i < nMask.length; i++){
				rMask |= nMask[0] << (8 * (nMask.length - i - 1));
			}
		}else{
			rMask = nMask;
		}
	...
	getMessageLength: function(buf){
		...
		if(length === undefined){
			if(buf[1] == CONSTANTS.PLM_COMMANDS.SEND_INSTEON){
				...
				// Not using INSTEONMessageFlags to avoid a cyclic dependency!
				length = util.parseBitmask(buf[5])[4] ? 21 : 7;
			}else{
				throw new Error("Received unrecognized command; something is FUBAR! BUF: " + buf.toString("hex"));
			}
		}
