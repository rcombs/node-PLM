...
var util = module.exports = {
	...
	stripUntilStartsWith: function(buf, starts){
	...
	getMessageLength: function(buf){
		...
		if(buf[1] >= CONSTANTS.MIN_OUT_COMMAND){
			// This is an echo/reply. Just look for an ACK/NAK byte.
			for(var i = 2; i < buf.length; i++){
				if(buf[i] == CONSTANTS.ACK || buf[i] == CONSTANTS.NAK){
					return i; // Found ACK/NAK; return everything up to it.
				}
			}
			return false;
		}
		// IM->Host commands are all fixed-length.
		// They're all also < 0x60. If this is one
		// of those, just see if we have enough bytes.
		var length = CONSTANTS.CMD_LEN[buf[1]];
