...
var PLM = module.exports = function PLM(path, cb){
	...
	this.on("message", function(buf){
		...
		if(buf[1] < CONSTANTS.MIN_OUT_COMMAND){
			...
			if(self.minCommandInterval){
				setTimeout(function(){
					self.busy = false;
					self.dequeue();
				}, self.minCommandInterval);
			}else{
				self.busy = false;
				self.dequeue();
			}
...
PLM.prototype.INSTEONDequeue = function INSTEONDequeue(){
	...
	this.sendINSTEONNow(cmd.message, function(err, data, cmdnum){
		...
		var timeout, callback = function(reply){
			...
			if(self.minINSTEONInterval){
				setTimeout(function(){
					self.INSTEONBusy = false;
					self.INSTEONDequeue();
				}, self.minINSTEONInterval);
			}else{
				self.INSTEONBusy = false;
				self.INSTEONDequeue();
			}
...
PLM.prototype.commandTimeout = PLM.prototype.minCommandInterval = 250;

PLM.prototype.minINSTEONInterval = 0; // This shouldn't be necessary, but provide the feature just in case
