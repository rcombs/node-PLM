...
var PLM = module.exports = function(path, cb){
	...
	this.on(CONSTANTS.CMDS.INSTEON_STD_RECV, handleMsgEvent);
	this.on(CONSTANTS.CMDS.INSTEON_EXT_RECV, handleMsgEvent);
...
PLM.prototype.sendINSTEON = function(message, cb){
	...
	msgbuf.copy(data, 1);
	this.sendCommand(data, function(ACK){
		if(!ACK){
			cb(new Error("PLMNAK"));
		}
	});
	this.once("ACK:" + (new Buffer(message.to)).toString("hex"), function(reply){
		if(reply.flags.NAK){
			cb(new Error("DEVNAK"), reply);
		}else{
			cb(false, reply);
		}
	});
